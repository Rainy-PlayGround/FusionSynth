  ...
  for _, v in processor_option {
		match v {
			...
			"2" {
				...
				processors_list << utils.eq_generator(wpf.sample_rate)
			}
			"3" {
				println('✓ Reverb Processor')
				processors_list << utils.reverb_generator(wpf.sample_rate)
			}
			"4" {
				println('✓ Compressor Processor')
				processors_list << utils.compressor_generator(wpf.sample_rate)
			}
			"5" {
